...
const WINDOW_STATE_FILE_VERSION: u8 = 1;
...
struct WindowStatePersistence {
    state: Mutex<WindowStatePersistenceState>,
}

#[derive(Default)]
struct WindowStatePersistenceState {
    data_dir: Option<PathBuf>,
    pending: Option<WindowSizeState>,
    persisted: Option<WindowSizeState>,
...
impl PersistedWindowState {
    fn from_size(size: WindowSizeState) -> Self {
        Self {
...
impl WindowStatePersistence {
    fn configure(&self, data_dir: PathBuf, restored: Option<WindowSizeState>) {
    ...
    fn request_debounced_save(self: &Arc<Self>, size: WindowSizeState) {
        let generation = {
            let mut state = self.state.lock().expect("window state lock poisoned");
            state.debounce_generation += 1;
            if state.persisted == Some(size) {
                state.pending = None;
                return;
            }
        ...
        let window_state = Arc::clone(self);
        std::thread::spawn(move || {
            std::thread::sleep(Duration::from_millis(WINDOW_STATE_SAVE_DEBOUNCE_MS));
            window_state.flush_generation(generation);
        });
    }

    fn save_now(&self, size: WindowSizeState) {
        let path = {
            let mut state = self.state.lock().expect("window state lock poisoned");
            state.debounce_generation += 1;
            state.pending = None;
            ...
            state.data_dir.as_ref().map(|data_dir| data_dir.join(WINDOW_STATE_FILE))
        };
        ...
        if write_window_state_file(&path, size).is_ok() {
            let mut state = self.state.lock().expect("window state lock poisoned");
            state.persisted = Some(size);
        }
    }
    ...
    fn flush_generation(&self, generation: u64) {
        let (path, size) = {
            ...
            let Some(size) = state.pending else {
                return;
            };
            if state.persisted == Some(size) {
                state.pending = None;
            ...
            let Some(path) = state.data_dir.as_ref().map(|data_dir| data_dir.join(WINDOW_STATE_FILE)) else {
                return;
            };
            (path, size)
        };
...
fn cbpanel_window_close(
    ...
    window_state: tauri::State<'_, Arc<WindowStatePersistence>>,
) -> Result<(), String> {
    save_window_size(&window, window_state.inner().as_ref());
...
/// Opens a native picker for an extension archive and returns the selected path verbatim; the
/// browser file input can only expose `C:\fakepath\<name>`, which no backend can read.
#[tauri::command]
async fn cbpanel_select_extension_archive_path(
    app: tauri::AppHandle,
    kind: String,
) -> Result<Option<String>, String> {
    let (title, filter_name, extensions) = match kind.as_str() {
        "zip" => (
            "Select extension ZIP package",
            "CBPanel Extension ZIP",
            EXTENSION_ZIP_EXTENSIONS,
        ),
        "crx" => (
            "Select extension CRX package",
            "CBPanel Extension CRX",
            EXTENSION_CRX_EXTENSIONS,
        ),
        _ => return Err(format!("Unsupported extension archive kind: {kind}.")),
    };
    tauri::async_runtime::spawn_blocking(move || {
        let Some(file_path) = app
            .dialog()
        ...
        let path = file_path
            .into_path()
            .map_err(|error| format!("Invalid extension package path: {error}."))?;
        Ok(Some(path.to_string_lossy().to_string()))
    })
    .await
    .map_err(|error| format!("Extension package picker task failed: {error}."))?
}

/// Reveals an extension's local directory in the system file manager.
///
/// The plugin's JS `open` command is always checked against the `plugins > shell > open` regex,
/// which defaults to `^((mailto:\w+)|(tel:\w+)|(https?://\w+)).+` and therefore rejects every
/// filesystem path. Calling the same plugin from Rust skips that scope check, so the directory
/// guard below is what keeps this command from turning into "launch an arbitrary local program".
#[tauri::command]
async fn cbpanel_open_directory(app: tauri::AppHandle, path: String) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || {
...
pub fn run() {
    ...
    let window_state = Arc::new(WindowStatePersistence::default());
    ...
    builder
        ...
        .setup(move |app| {
            ...
            let runtime = prepare_runtime(app, window_state.as_ref());
            let _ = setup_tray(app.handle());
            if let Some(window) = app.get_webview_window("main") {
                let event_window = window.clone();
                let event_window_state = Arc::clone(&window_state);
                window.on_window_event(move |event| {
                    match event {
                        WindowEvent::Resized(_) => {
                            schedule_window_size_save(&event_window, &event_window_state);
                        }
                        WindowEvent::CloseRequested { .. } | WindowEvent::Destroyed => {
                            save_window_size(&event_window, &event_window_state);
                        }
                        _ => {}
                    }
                });
...
fn prepare_runtime(app: &mut tauri::App, window_state: &WindowStatePersistence) -> RuntimeState {
    ...
    restore_main_window_size(app, window_state, &data_dir);
...
fn restore_main_window_size(
    app: &mut tauri::App,
    window_state: &WindowStatePersistence,
    data_dir: &Path,
) {
    let size = read_window_size_state(data_dir);
    window_state.configure(data_dir.to_path_buf(), size);
...
fn save_main_window_size(app_handle: &tauri::AppHandle, window_state: &WindowStatePersistence) {
    let Some(window) = app_handle.get_webview_window("main") else {
        return;
    };
    save_window_size(&window, window_state);
}

fn save_window_size(window: &tauri::WebviewWindow, window_state: &WindowStatePersistence) {
    let Some(size) = capture_window_size(window) else {
        return;
    };
    window_state.save_now(size);
}

fn schedule_window_size_save(window: &tauri::WebviewWindow, window_state: &Arc<WindowStatePersistence>) {
    let Some(size) = capture_window_size(window) else {
        return;
    };
    window_state.request_debounced_save(size);
}

fn capture_window_size(window: &tauri::WebviewWindow) -> Option<WindowSizeState> {
    if window.is_minimized().unwrap_or(false) || window.is_maximized().unwrap_or(false) {
        return None;
    }
    let size = window.inner_size().ok()?;
    window_size_state_from_physical(size)
}

fn window_size_state_from_physical(size: tauri::PhysicalSize<u32>) -> Option<WindowSizeState> {
...
fn write_window_state_file(path: &Path, size: WindowSizeState) -> Result<(), String> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|error| format!("Could not create window state directory {}: {error}.", parent.display()))?;
    }

    let mut payload = serde_json::to_vec_pretty(&PersistedWindowState::from_size(size))
        .map_err(|error| format!("Could not serialize window state: {error}."))?;
    payload.push(b'\n');
    std::fs::write(path, payload)
        .map_err(|error| format!("Could not write window state {}: {error}.", path.display()))
}
...
fn read_window_size_state(data_dir: &Path) -> Option<WindowSizeState> {
    ...
    deserialize_window_size_state(&raw)
}

fn deserialize_window_size_state(raw: &str) -> Option<WindowSizeState> {
    let state = serde_json::from_str::<PersistedWindowState>(raw).ok()?;
    if state.version != WINDOW_STATE_FILE_VERSION {
        return None;
    }
    Some(WindowSizeState {
...
mod tests {
    ...
    #[test]
    fn deserialize_window_size_state_rejects_legacy_text_payload() {
        assert_eq!(deserialize_window_size_state("1440,900"), None);
    }

    #[test]
    fn deserialize_window_size_state_clamps_below_minimum_dimensions() {
        let size = deserialize_window_size_state(r#"{"version":1,"width":900,"height":500}"#)
            .expect("window state should parse");
    ...
    #[test]
    fn deserialize_window_size_state_rejects_unknown_versions() {
        assert_eq!(
