...
export type BrowserCoreReleaseChannel = "stable" | "preview";
...
export const DEFAULT_APP_SETTINGS: AppSettings = {
  ...
  binary: {
    ...
    tierMode: "free",
    licenseKey: "",
    // Off, because a fresh install has no key: "enabled" over an empty key only promised a licence that
    // was never configured. The panel keeps the key field disabled until this is on, so turning it on is
    // the deliberate step that starts sending a key to the licence server. Anyone who already stored a
    // preference keeps it — normalizeSettings falls back to this default only when the field is absent.
...
export function normalizeSettings(input: AppSettingsPatch = {}): AppSettings {
  ...
  const licenseKey = normalizeLicenseKey(binary.licenseKey, base.binary.licenseKey);
  ...
  return {
    ...
    binary: {
      ...
      licenseKey,
      // The switch is younger than the key field, so settings written before it existed carry a key and
      // no switch. Falling through to the shipped default would read that as "off" and drop the operator
      // to the free build on upgrade, with their key still shown in the panel — the one thing this
      // default must not do. A key on file *is* the old behaviour's "on", so it stands in for the
      // missing field; an absent switch over no key still lands on the default.
      licenseKeyEnabled: booleanValue(binary.licenseKeyEnabled ?? (licenseKey ? true : undefined), base.binary.licenseKeyEnabled),
      ...
      releaseChannel: enumValue(binary.releaseChannel, ["stable", "preview"], base.binary.releaseChannel),
...
function normalizeBrowserCoreUpdateCheck(value: unknown): BrowserCoreUpdateCheck | undefined {
  ...
  return {
    ...
    // A code, so an unknown one is dropped rather than carried into the panel as an untranslatable
    // caveat. Dropping only the caveat and keeping the check is deliberate: the comparison itself is
    // still the last thing the server actually did.
    baselineCaveat: optionalEnumValue<NonNullable<BrowserCoreUpdateCheck["baselineCaveat"]>>(
      input.baselineCaveat,
      ["offline-import-tier-mismatch"],
    ),
