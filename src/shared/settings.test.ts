...
test("normalizeSettings keeps the release channel stable unless preview is chosen", () => {
  assert.equal(normalizeSettings().binary.releaseChannel, "stable");
  assert.equal(
    normalizeSettings({ binary: { ...DEFAULT_APP_SETTINGS.binary, releaseChannel: "preview" } }).binary.releaseChannel,
    "preview",
  );
  assert.equal(
    normalizeSettings({
      binary: { ...DEFAULT_APP_SETTINGS.binary, releaseChannel: "nightly" as never },
    }).binary.releaseChannel,
    "stable",
  );
});

test("normalizeSettings preserves a stored license switch on both sides of its default", () => {
  // The switch now defaults to off, and normalizeSettings runs over every read, so the fallback is the
  // only thing standing between a stored preference and a silent flip. It has to be reached solely when
  // the field is absent: an operator who turned the key on must not find it off after an upgrade, and one
  // who turned it off must not find it on.
  assert.equal(normalizeSettings().binary.licenseKeyEnabled, false);
  assert.equal(
    normalizeSettings({ binary: { ...DEFAULT_APP_SETTINGS.binary, licenseKeyEnabled: true } }).binary.licenseKeyEnabled,
    true,
  );
  assert.equal(
    normalizeSettings({ binary: { ...DEFAULT_APP_SETTINGS.binary, licenseKeyEnabled: false } }).binary.licenseKeyEnabled,
    false,
  );
  // An explicit switch always wins, key or no key: the two are stored, and reported, separately.
  assert.equal(
    normalizeSettings({ binary: { ...DEFAULT_APP_SETTINGS.binary, licenseKey: "cb_stored" } }).binary.licenseKeyEnabled,
    false,
  );
});

test("normalizeSettings reads a key stored before the switch existed as an enabled one", () => {
  // The switch was added after the key field, so settings written in between carry a key and no switch.
  // Reading that as the shipped "off" would drop a paying install to the free build on upgrade while the
  // panel still shows the key — the key is what the old behaviour acted on, so it stands in for the
  // field that is missing.
  assert.equal(normalizeSettings({ binary: { licenseKey: "cb_stored" } }).binary.licenseKeyEnabled, true);
  assert.equal(normalizeSettings({ binary: { licenseKey: "cb_stored" } }).binary.licenseKey, "cb_stored");
  // Only a key does that. An absent switch over no key is a fresh install, which is the case the default
  // exists for, and a whitespace-only key is no key.
  assert.equal(normalizeSettings({ binary: {} }).binary.licenseKeyEnabled, false);
  assert.equal(normalizeSettings({ binary: { licenseKey: "   " } }).binary.licenseKeyEnabled, false);
  // And an explicit off is never overridden by the key beside it.
  assert.equal(
    normalizeSettings({ binary: { licenseKey: "cb_stored", licenseKeyEnabled: false } }).binary.licenseKeyEnabled,
    false,
  );
});
...
test("normalizeSettings preserves valid browser core update check state", () => {
  const settings = normalizeSettings({
    binary: {
      ...
      lastUpdateCheck: {
        ...
        targetTier: "pro",
        versionMode: "latest",
        baselineCaveat: "offline-import-tier-mismatch",
      },
    },
  });
  ...
  assert.equal(settings.binary.lastUpdateCheck?.targetTier, "pro");
  // The caveat is the reason the panel does not present the verdict as settled, so losing it on restart
  // would restore a confident "up to date" the server never meant.
  assert.equal(settings.binary.lastUpdateCheck?.baselineCaveat, "offline-import-tier-mismatch");
});
...
test("normalizeSettings drops invalid browser core update check state", () => {
  ...
  // An unknown caveat code costs only the caveat: the comparison itself is still the last thing the
  // server did, and the panel translates the code, so carrying one it cannot translate is the only loss.
  const unknownCaveat = normalizeSettings({
    binary: {
      ...DEFAULT_APP_SETTINGS.binary,
      lastUpdateCheck: {
        checkedAt: "2026-06-06T00:00:00.000Z",
        ...
        baselineCaveat: "made-up-code",
      } as never,
    },
  });

  assert.equal(unknownCaveat.binary.lastUpdateCheck?.currentVersion, "146.0.7680.177.5");
  assert.equal(unknownCaveat.binary.lastUpdateCheck?.baselineCaveat, undefined);
});
