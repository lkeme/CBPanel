...
function networkCheckPreflight(check: NetworkCheckResult | undefined): ProfilePreflightItem {
  if (!check) {
    return {
      ...
      severity: "warn",
      title: "出口检查",
      detail: "尚未记录代理出口检查；建议先检查代理，确认出口 IP、地区代码和检测端点。",
  ...
  return {
    ...
    title: "出口检查",
    detail: [
      `出口 IP：${check.ip ?? "unknown"}`,
      check.latencyMs !== undefined ? `延迟：${check.latencyMs}ms` : "",
      traceParts.length ? `Trace：${traceParts.join(" / ")}` : "",
    ].filter(Boolean).join("；"),
  };
}
