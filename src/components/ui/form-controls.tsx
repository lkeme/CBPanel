...
export function Drawer({
  ...
  // role="dialog" takes its name from the author, so the heading has to be linked explicitly:
  // an unassociated <h2> leaves the drawer announced as a bare dialog.
  const titleId = React.useId();
  // Escape closes, matching the scrim and the header button. The listener sits on window because
  // focus is on <body> right after the drawer opens, so a handler on the panel would never fire.
  // Nested popovers stopPropagation while they are open, so Escape dismisses the innermost first.
  ...
  return (
    <div className="drawer-layer" role="dialog" aria-modal="true" aria-labelledby={titleId}>
      ...
      <section className="drawer-panel">
        <header className="drawer-header">
          ...
          {actions}
          <button className="icon-button" aria-label={t("actions.close")} title={t("actions.close")} onClick={close} type="button">
            <X size={18} />
          </button>
        </header>
