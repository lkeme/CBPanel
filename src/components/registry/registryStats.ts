...
export function buildModuleStats(state: PanelState | null, sessionsByProfileId: Map<string, SessionSummary>): ModuleStats {
      ...
      ? sortStats(
          state.proxies.map((proxy) => {
            ...
            return {
              ...
              count: related.length,
              running: related.reduce((count, environment) => count + runningByEnvironmentId(environment.id), 0),
              description: networkCheckPlainText(proxy.lastCheck),
              status: proxy.status,
            };
          }),
        )
...
function networkCheckPlainText(check: NetworkCheckResult | undefined): string | undefined {
