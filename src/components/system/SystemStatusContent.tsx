      ...
      <div className="settings-stack system-status-stack">
        ...
        <section className="settings-section">
          <div className="settings-section-head">
            ...
            {/* Permanent delete used to leave `browser-data/<id>` behind, so existing installs carry orphan
                directories that fixing the delete path alone cannot reclaim. */}
            <button
              className="command danger subtle"
              disabled={busy === "browser-data-prune"}
              onClick={() => void pruneBrowserData()}
              type="button"
            >
              <Trash2 size={16} aria-hidden="true" />
              {t("actions.pruneBrowserData")}
            </button>
          </div>
          <KeyValueList
            items={[
