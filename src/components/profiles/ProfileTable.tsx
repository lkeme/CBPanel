...
function renderCell({
  ...
  switch (columnId) {
    ...
    case "ip":
      return <ExitCell profile={profile} check={environment?.lastNetworkCheck ?? proxy?.lastCheck} t={t} />;
    case "mode":
...
function ExitCell({
  ...
  const info = exitCellInfo(profile, check, t);
  return (
    <span className={`ip-cell ${info.tone}`} title={exitCellTitle(profile, check, t)}>
...
function exitCellInfo(
  ...
  if (check?.ok) {
    const region = [check.trace?.loc, check.trace?.colo].filter(Boolean).join(" / ");
    return {
      primary: region || check.trace?.providerName || t("table.ipUnchecked"),
...
function exitCellTitle(
  ...
  const proxy = buildProxyUrl(profile.proxy);
  const parts = proxy
    ? [t("table.proxy"), maskProxyUrlForDisplay(proxy), networkCheckSummaryText(check, t)]
    : [t("table.directConnection"), networkCheckSummaryText(check, t)];
...
export function networkCheckSummaryText(
  ...
  if (!check) return t("table.ipUnchecked");
  if (!check.ok) return check.error || t("table.ipCheckFailed");
  const parts = [
    check.ip ? t("network.ip", { ip: check.ip }) : "",
    check.latencyMs !== undefined ? t("network.latency", { latency: check.latencyMs }) : "",
    check.trace?.loc ? t("networkTrace.loc", { loc: check.trace.loc }) : "",
    check.trace?.colo ? t("networkTrace.colo", { colo: check.trace.colo }) : "",
    check.trace?.providerName ? t("networkTrace.providerValue", { provider: check.trace.providerName }) : "",
  ].filter(Boolean);
  return parts.join(" / ") || t("proxy.check.ok", { latency: check.latencyMs ?? "-" });
}
