...
export function useRegistryActions({
  ...
  async function clearTrashEnvironmentsNow() {
    setBusy("trash-clear");
    try {
      // `dataRemoved` is what makes this report honest: rows and `browser-data/<id>` directories are deleted
      // by two different layers, and only the second one can be refused by the OS.
      const result = await api<{ deleted: number; dataRemoved: number; warnings: string[] }>("/api/trash/environments", {
        method: "DELETE",
      });
      setConfirmDialog(null);
      await loadState();
      toast(
        "success",
        t("toast.trashCleared", { count: result.deleted, dataRemoved: result.dataRemoved, warnings: result.warnings.length }),
      );
      reportEnvironmentDataWarnings(result.warnings);
    } catch (error) {
      toast("error", (error as Error).message);
    } finally {
      setBusy("");
  ...
  async function pruneBrowserData() {
    setConfirmDialog({
      title: t("actions.pruneBrowserData"),
      body: t("confirm.pruneBrowserData"),
      confirmLabel: t("actions.pruneBrowserData"),
      tone: "danger",
      busyKey: "browser-data-prune",
      onConfirm: pruneBrowserDataNow,
    });
  }

  async function pruneBrowserDataNow() {
    setBusy("browser-data-prune");
    try {
      const result = await api<{ removed: string[]; warnings: string[] }>("/api/storage/browser-data/prune", { method: "POST" });
      setConfirmDialog(null);
      await loadState();
      if (result.removed.length === 0 && result.warnings.length === 0) {
        // Finding nothing is the normal outcome on a healthy install, and reporting it as a cleanup would
        // teach the user that the button always reclaims something.
        toast("info", t("toast.browserDataPruneEmpty"));
      } else {
        toast(
          result.removed.length > 0 ? "success" : "info",
          t("toast.browserDataPruned", { count: result.removed.length, warnings: result.warnings.length }),
        );
      }
      reportEnvironmentDataWarnings(result.warnings);
    } catch (error) {
  ...
  /**
   * A directory the OS refused to delete is the one thing the counts cannot express, and staying silent
   * about it leaves reclaimable disk space with no explanation. Only the first is shown, as the backup and
   * package flows do — the rest repeat the same story per directory — and it never downgrades the outcome,
   * because the rows are already gone.
   */
  function reportEnvironmentDataWarnings(warnings: string[]) {
    const warning = warnings[0];
    if (warning) toast("info", t("toast.environmentDataWarning", { message: warning }));
  }
