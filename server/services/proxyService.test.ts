...
test("ProxyService accepts Cloudflare trace responses with only ip", async () => {
  const service = new ProxyService({
  ...
  assert.equal(result.ip, "203.0.113.42");
  assert.equal(result.trace?.providerId, "cloudflare-www");
  assert.equal(result.trace?.loc, undefined);
  assert.equal(result.trace?.colo, undefined);
});
