...
if (!isMacRustTarget(rustTarget)) {
  throw new Error(`macOS release smoke only supports apple-darwin targets. Current target: ${rustTarget}`);
}
...
const dmgPath = path.join(root, "release", `CBPanel-macos-${artifactArch}.dmg`);
const manifestPath = path.join(root, "release", "app-update-manifest.json");
const smokeDataDir = path.join(root, "release", "mac-smoke-data");
...
await assertExists(sidecarPath, "Missing generated macOS sidecar executable.");
await assertExists(dmgPath, "macOS DMG is missing.");
await assertExists(manifestPath, "macOS update manifest is missing.");
await assertManifestContainsMacDmg(manifestPath, path.basename(dmgPath));
...
try {
  ...
  if (runtime.shell !== "desktop" || runtime.platform !== "macos") {
    throw new Error(`Unexpected macOS runtime payload: ${JSON.stringify(runtime)}`);
  }
  ...
  console.log("macOS release smoke passed: sidecar auth, runtime API, binary API, packaged dependencies, SQLite state, DMG, and manifest are present.");
} finally {
  child.kill();
  await sleep(500);
...
async function assertManifestContainsMacDmg(inputPath, fileName) {
  const manifest = JSON.parse(await fs.readFile(inputPath, "utf8"));
  const artifact = manifest.artifacts?.find((item) => item.kind === "macos-dmg" && item.fileName === fileName);
  if (!artifact || typeof artifact.sha256 !== "string" || typeof artifact.size !== "number" || artifact.size <= 0) {
    throw new Error(`Update manifest does not include a valid macOS DMG artifact for ${fileName}.`);
  }
}
...
async function waitForReady(inputPort, inputToken) {
  ...
  throw new Error(`macOS sidecar did not become ready: ${lastError?.message ?? "timeout"}`);
}
