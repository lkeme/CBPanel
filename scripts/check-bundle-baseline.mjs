...
function classifyChunk(fileName) {
  ...
  if (/^locale-.*\.js$/.test(fileName)) return "locale";
  // Left unanchored on purpose: this one has to match react-vendor-, radix-vendor-, ui-vendor-, …
  ...
  // Anchored, or this would swallow ColumnSettingsDrawer and make the branch below unreachable.
...
/**
 * Strips Vite's content hash so a chunk keeps one identity across builds. Comparing by file name
 * cannot work: the hash changes precisely when the content does, so every chunk worth checking would
 * look like a new one. Falls back to the full name when the suffix does not look like a hash, which
 * degrades to the old behaviour instead of keying two different chunks together.
 */
function stableChunkName(fileName) {
...
async function collectChunks() {
  ...
  for (const entry of entries) {
    ...
    chunks.push({
      file: entry.name,
      name: stableChunkName(entry.name),
...
function compareAgainstBaseline(current, previous) {
  ...
  const failures = [];
  // Baselines written before chunks carried a stable name still key fine: derive it from the file.
  const previousChunks = byKey(
    (previous.chunks ?? []).map((chunk) => ({ ...chunk, name: chunk.name ?? stableChunkName(chunk.file) })),
    "name",
  );
  ...
  for (const chunk of current.chunks) {
    const old = previousChunks.get(chunk.name);
    if (!old) {
      // Its growth is still covered by the role aggregate, but a chunk appearing out of nowhere
    ...
    if (rawGrowth > limits.perChunkBytes) {
      failures.push(`${chunk.name} grew by ${kb(rawGrowth)} KB raw; limit is ${kb(limits.perChunkBytes)} KB`);
    }
    if (chunk.role === "entry") {
      const gzipGrowth = chunk.gzipBytes - old.gzipBytes;
      if (rawGrowth > limits.entryBytes) {
        failures.push(`entry chunk ${chunk.name} grew by ${kb(rawGrowth)} KB raw; limit is ${kb(limits.entryBytes)} KB`);
      }
      if (gzipGrowth > limits.entryGzipBytes) {
        failures.push(`entry chunk ${chunk.name} grew by ${kb(gzipGrowth)} KB gzip; limit is ${kb(limits.entryGzipBytes)} KB`);
      }
    }
  }

  if (introduced.length > 0) {
    console.log(`New chunks since the baseline (size checked via their role total): ${introduced.map((chunk) => `${chunk.name} ${kb(chunk.bytes)} KB`).join(", ")}`);
  }
